...
function updateStrengthIndicator(strength) {
    ...
    strengthText.title = `Score: ${strength.score}/7 - Features: ${strength.details.join(", ")}`;
...
function copyToClipboard() {
    ...
    if (passEl) {
        navigator.clipboard.writeText(passEl)
            .then(() => {
...
function displayPasswordHistory() {
    ...
    if (passwordHistory.length === 0) {
        ...
        emptyMessage.className = 'text-gray-400 dark:text-gray-500 text-center py-2 text-xs italic';
    ...
    passwordHistory.forEach(password => {
        ...
        historyItem.className = 'flex items-center justify-between py-1.5 border-b border-gray-100 dark:border-gray-700 last:border-b-0';
        ...
        useButton.className = 'text-primary-DEFAULT dark:text-primary-light hover:text-primary-dark';
        ...
        deleteButton.addEventListener('click', () => {
            ...
            const updatedHistory = JSON.parse(localStorage.getItem('passwordHistory') || '[]')
                .filter(p => p !== password);
